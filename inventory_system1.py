...
# FIX E501: Broke long line
# FIX C0103: Changed to UPPER_CASE
...
def add_item(stock_data, item, qty, logs=None):
    ...
    if not isinstance(item, str) or not isinstance(qty, int):
        # FIX E501: Broke long line
        logging.warning(
            "Invalid types passed to add_item: item=%s (%s), qty=%s (%s)",
...
def remove_item(stock_data, item, qty):
    ...
    try:
        ...
        if stock_data[item] <= 0:
            del stock_data[item]
            # FIX E501: Broke long string
            logging.info("Removed item %s from stock (quantity reached 0).",
                         item)
...
def load_data(file="inventory.json"):
    ...
    try:
        ...
        # FIX E501: Broke long string
        logging.warning("Inventory file %s not found. "
                        "Starting with empty inventory.", file)
        return {}
    except json.JSONDecodeError:
        # FIX E501: Broke long string
        logging.error("Could not decode JSON from %s. "
                      "Starting with empty inventory.", file)
        return {}
...
def save_data(stock_data, file="inventory.json"):
    ...
    try:
        ...
        # **FIX for E1206: Added 'file' argument back**
